...
* 语言版本
```
现代的PHP组件都使用语义版本方案(http://semver.org/)，版本号由三个点(.)分数字组成(例如1.13.2)。第一个数字是主版本号，如果PHP组件的更新破坏了向后兼容性，会提升主版本号。第二个数字是次版本号，如果PHP组件小幅更新了功能，而且没破坏向后兼容性，会提升次版本号。第三个数字是修订版本号，如果PHP修正了向后兼容的缺陷，会提升修订版本号。
```
